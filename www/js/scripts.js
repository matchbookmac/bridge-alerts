  ...
  var slideout = new Slideout({
    ...
    $( "#morrison").click(function(){
      ...
      $.getJSON( "http://54.191.150.69/bridges/morrison/events/actual/5", function( data ) {
        var items = [];
        $.each( data, function( key, val ) {
          // var up_time = val.up_time;
          var up_time = JSON.parse(val.up_time);
          console.log("json parsed: " + up_time);
          up_time = new Date(parseInt(val.up_time.substr(6)));
          console.log(up_time);
          up_time = $.datepicker.formatDate("mm/dd/yyyy", up_time);
          console.log("formatted : " + up_time)

          var down_time = Date(val.down_time);
          down_time = down_time.slice(0,24);
          items.push("<li class='bridge-event' style='text-align: center;'>" + " Time up: " + up_time + "</li>" );
          items.push("<li class='bridge-event' style='text-align: center;'>" + "Time down: " + down_time + "</li><br>" );
        });
